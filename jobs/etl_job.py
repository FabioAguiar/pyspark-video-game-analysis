...
Script para execução do job Spark:
    $SPARK_HOME/bin/spark-submit \
        --master local[*] \
        --py-files packages.zip $HOME/projects/pyspark-video-game-analysis/jobs/etl_job.py
"""
...
def main():
    ...
    #data_transformed = transform_data(data, "COUNT_GENRES", plataforms)
    plataforms = data['Platform'].unique().to_list()
    load_plot_pie_1(transform_data(data, "COUNT_GENRES"), plataforms, "COUNT_GENRES")
    load_plot_pie_2(transform_data(data, "SUM_GLOBAL_SALES"), plataforms, "SUM_GLOBAL_SALES")
    load_plot_bar_1(transform_data(data, "COUNT_GENRES"), plataforms) 
    load_plot_bar_2(transform_data(data, "SUM_GLOBAL_SALES"), plataforms) 
    load_plot_bar_3(transform_data(data, "COUNT_GAMES_YEAR"), plataforms) 
...
def load_plot_pie_1(df, plataforms, chart_metric):
    for platform in plataforms:
        ...
        fig = plot.get_figure()
        fig.savefig("../pyspark-video-game-analysis/data/plots/"+"Pie_Count_Genre_percent_"+platform+".png")

    return None


def load_plot_pie_2(df, plataforms, chart_metric):
    for platform in plataforms:
        pie_df = df.xs(platform,level=0)
        pie_df = round_value_chart(pie_df, chart_metric) #Arredonda registros do gráfico retirando os menores valores
        ...
        fig = plot.get_figure()
        fig.savefig("../pyspark-video-game-analysis/data/plots/"+"Pie_Global_Sales_Genre_percent_"+platform+".png")

    return None

def load_plot_bar_1(df, plataforms):
    for platform in plataforms:
        pie_df = df.xs(platform,level=0)
        plot = pie_df.to_pandas().plot.pie(y='Genre', figsize=(10,10), title=("Contagem de jogos por gênero: " + platform), fontsize=16, pctdistance=0.9, autopct='%1.1f%%',legend=False)
        fig = plot.get_figure()
        fig.savefig("../pyspark-video-game-analysis/data/plots/"+"Bar_Count_Genre_percent_"+platform+".png")

    return None

def load_plot_bar_2(df, plataforms):
    for platform in plataforms:
        pie_df = df.xs(platform,level=0)
        ...
        fig = plot.get_figure()
        fig.savefig("../pyspark-video-game-analysis/data/plots/"+"Bar_Global_Sales_Genre_"+platform+".png")

    return None

def load_plot_bar_3(df, plataforms):
    for platform in plataforms:
        pie_df = df.xs(platform,level=0)
        ...
        fig = plot.get_figure()
        fig.savefig("../pyspark-video-game-analysis/data/plots/"+"Bar_count_Genre_per_year"+platform+".png")
